        ...
        rules: [
          {
            ...
            annotations: {
              ...
              description: 'The Puppet CA on {{ $labels.instance }} has {{ $labels.subject }} configured in managed_certs or serving_cert but no certificate for it exists after %(managedCertNeverIssuedFor)s. Whatever depends on that certificate has nothing to present. Check the CA logs for the managed-certificate reconcile pass and for the store it writes to -- a Secret refused by RBAC, an unadoptable Secret holding somebody else\'s material, or a directory that does not exist. If the subject is the CA\'s own certname it came from serving_cert, and the CA would not have started at all had it never been issued -- so a certificate that existed no longer has a live inventory row. The rule fires on the absence of a non-revoked row, so revocation is the likeliest cause but not the only one -- check the CRL first, then the inventory itself.' % { managedCertNeverIssuedFor: $._config.managedCertNeverIssuedFor },
            },
          },
        ],
      },
