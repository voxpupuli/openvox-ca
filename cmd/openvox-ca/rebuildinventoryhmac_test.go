...
// appendingBackend grows the inventory between the report and the rebuild, the
// way a concurrent writer does while the operator is reading the report and
// deciding. The first read is what the operator was shown; every read after it
// sees an extra entry.
...
func (b *appendingBackend) Get(ctx context.Context, key string) ([]byte, error) {
	...
	if b.reads > 1 {
		data = append(data,
			[]byte("0x0777 2026-01-01T00:00:00UTC 2036-01-01T00:00:00UTC /CN=appended-after-the-report\n")...)
	}
	return data, nil
}
...
var _ = Describe("headFingerprint", func() {
	// The command only ever renders 32-byte heads, so its <=8 branch is
	// unreachable through any end-to-end spec. Driven directly, because a
	// rendering helper that silently truncated a short value -- or indexed
	// past the end of one -- would be found by an operator comparing two
	// fingerprints by eye, not by the suite.
	DescribeTable("renders a value short enough not to need truncating, in full",
		func(head []byte, want string) {
			Expect(headFingerprint(head, "(absent)")).To(Equal(want))
		},
		Entry("one byte", []byte{0xab}, "ab"),
		Entry("exactly the truncation boundary", []byte{1, 2, 3, 4, 5, 6, 7, 8}, "0102030405060708"),
	)

	It("truncates and marks anything longer", func() {
		nine := []byte{1, 2, 3, 4, 5, 6, 7, 8, 9}
		Expect(headFingerprint(nine, "(absent)")).To(Equal("0102030405060708…"),
			"the boundary is inclusive below and exclusive above, so nine bytes is the first truncated case")
	})

	It("returns the caller's word for absent rather than an empty rendering", func() {
		Expect(headFingerprint(nil, "(absent)")).To(Equal("(absent)"))
		Expect(headFingerprint([]byte{}, "(absent)")).To(Equal("(absent)"))
	})
})

var _ = Describe("fullHead", func() {
	// The audit record's rendering, and the durable account of what a rebuild
	// replaced. It renders in full rather than truncating, because a log line
	// has no width to save and eight of thirty-two bytes does not answer "what
	// was signed over".
	It("distinguishes a value that is absent from one that is stored and empty", func() {
		// A torn write leaves a zero-length value. Rendering it as "none" said
		// no value was stored when one was -- in the record that outlives the
		// terminal, which is the worse place for that conflation than the
		// report, where storedFingerprint already avoids it.
		Expect(fullHead(nil)).To(Equal("none"))
		Expect(fullHead([]byte{})).To(Equal("stored-but-empty"))
		Expect(fullHead([]byte{})).NotTo(Equal(fullHead(nil)),
			"the two must not collapse onto one rendering")
	})

	It("renders a real value in full, untruncated", func() {
		full := make([]byte, 32)
		for i := range full {
			full[i] = byte(i)
		}
		Expect(fullHead(full)).To(HaveLen(64))
		Expect(fullHead(full)).To(HavePrefix("000102030405"))
	})
})
...
var _ = Describe("openvox-ca rebuild-inventory-hmac", func() {
	...
	Describe("what it never reaches for", func() {
		It("does not open a session to the CA key backend", func() {
			// The security-relevant default: resolveRuntime is called with
			// withKeyProvider false, because this command reads the inventory
			// and the inventory HMAC key, both of which live in storage. Pinned
			// behaviourally rather than by reading the argument, so a refactor
			// that flips it fails here.
			//
			// The OpenBao address is unreachable on purpose: reaching it is not
			// the point, trying to is. csr passes true and fails against this
			// same config; this command must not.
			...
			// Asserted as nil, not as "not an OpenBao error": MatchError on a
			// nil error is itself a failure, and the property is that nothing
			// went wrong at all.
			Expect(err).NotTo(HaveOccurred(),
				"a provider this command never builds cannot fail it")
	...
	Describe("what it refuses to do", func() {
		...
		Describe("where the single-instance rule cannot be enforced", func() {
			...
			It("refuses when the capability could not be determined at all", func() {
				...
				// The assertion its three sibling refusals carry, and the one
				// that makes this a refusal rather than a message: a gate that
				// reported correctly and wrote anyway would pass everything
				// above.
				Expect(startCA(caDir)).To(MatchError(storage.ErrInventoryTampered),
					"a refusal must leave the store untouched")
			})
		...
		It("does not promise a mint when there is no value to rebuild", func() {
			// The wrong-length arm fires before the no-baseline arm, so a store
			// with BOTH an unusable key and no stored value was told
			// "Rebuilding will mint a new key as well as a new value" and then
			// took the nothing-to-rebuild path and returned without touching
			// the key. Found by mutation: removing the guard left the suite
			// green, because nothing asserted on this combination.
			Expect(os.WriteFile(filepath.Join(caDir, "private/.inventory_hmac_key"),
				[]byte("far too short"), 0o600)).To(Succeed())
			before, readErr := os.ReadFile(filepath.Join(caDir, "private/.inventory_hmac_key"))
			Expect(readErr).NotTo(HaveOccurred())
			Expect(os.Remove(filepath.Join(caDir, ".inventory.hmac"))).To(Succeed())
			...
			Expect(stdout).To(ContainSubstring("HMAC key:      wrong-length"))
			Expect(stdout).NotTo(ContainSubstring("Rebuilding will mint a new key"),
				"nothing is rebuilt on this path, so nothing is minted")
			Expect(stdout).To(ContainSubstring("will not replace the key"))

			after, readAfter := os.ReadFile(filepath.Join(caDir, "private/.inventory_hmac_key"))
			Expect(readAfter).NotTo(HaveOccurred())
			Expect(after).To(Equal(before),
				"and the promise is kept: the unusable key is still there")
		})
		...
		It("refuses to call it done when the inventory changed after the report", func() {
			// The operator reads a report, decides, and confirms. Between the
			// two, something appends. Without binding, the command blesses the
			// larger inventory and reports success, so the value covers content
			// that never appeared in the account that was approved.
			logFile := writeLoggingConfig()
			breakInventoryIntegrity(caDir)
			store := storeOver(caDir, func(b storage.Backend) storage.Backend {
			...
			Expect(err).To(MatchError(ContainSubstring("content you were not shown")))
