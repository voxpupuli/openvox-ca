...
is the safe way to inspect a CA that will not start.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Bind the blessing to the state the operator was shown. The report
			// above is what they read and confirmed; the value just written
			// covers the inventory as it stood at the write. Between the two
			// sits operator decision time, and anything appended in it is
			// signed over without ever appearing in the account that was
			// approved.
			//
			...
			if rep.KeyState == storage.HMACKeyUsable && !bytes.Equal(after.StoredHead, rep.ComputedHead) {
				...
				return fmt.Errorf("the inventory changed after it was reported: the report described a state "+
					"digesting to %s, and the value now stored covers %s. Something appended to this store "+
					"between the report and the rebuild, so the integrity value now covers content you were "+
					"not shown. Re-run to see the current state before deciding again",
	...
	// --help disagree with itself across the family. No shorthand, for the same
	// reason: none of the three siblings has one, nor does openvox-ca-ctl's own
	// --config.
	...
	// Named from storage.RebuildConfirmFlag, which also builds the repair hint
	// on the integrity-mismatch log lines: one definition, so the flag an
	// operator is told to use is the flag that exists.
	f.BoolVar(&confirm, storage.RebuildConfirmFlag, false,
...
func reportInventoryIntegrity(w io.Writer, rep storage.InventoryIntegrityReport) {
	...
	case rep.KeyState == storage.HMACKeyWrongLength:
		...
		_, _ = fmt.Fprintf(w, "\nThe stored HMAC key is the wrong length, so every existing value was computed\n"+
			"under a key that no longer exists and none of them can be reproduced.\n")
		// Only promise the mint on the path that performs it. With no value
		// stored there is nothing to rebuild, so the command reports that and
		// returns without touching the key -- and saying otherwise here told an
		// operator their key was about to be replaced when it was not.
		if rep.StoredHead != nil {
			_, _ = fmt.Fprintf(w, "Rebuilding will mint a new key as well as a new value.\n")
		} else {
			_, _ = fmt.Fprintf(w, "No value is stored either, so there is nothing to rebuild and this command\n"+
				"will not replace the key. Starting the server mints one and establishes a baseline.\n")
		}
	case rep.Verifies:
...
func fullHead(head []byte) string {
	...
	case head == nil:
		return "none"
	case len(head) == 0:
		// Present and zero length, which a torn write leaves behind. "none"
		// here would say no value was stored when one was, in the record that
		// outlives the terminal -- the same conflation storedFingerprint exists
		// to avoid on the report, and worse here because this is the durable
