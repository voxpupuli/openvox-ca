...
Three properties vary by backend and are not visible from the `Backend`
interface alone. `StorageService` answers both, because callers that need to know cannot
...
The third is `LockIsEnforced`, which answers a question the other two cannot:
whether an instance lock that was successfully taken actually excludes anybody.
...
convert "cannot be shown to be quiet" into "proceed".
