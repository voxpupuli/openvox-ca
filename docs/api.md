...
A revocation (`"desired_state": "revoked"`) takes the per-subject lock that signing and [renewal](#certificate-renewal) take, so it waits for an issuance already under way for that subject instead of overlapping it. That is what stops an in-flight renewal minting a replacement from the credential being withdrawn. If it waits out the 60-second lock timeout it answers `503` with `Retry-After` rather than `409`; revoking a serial already on the CRL is a no-op, so the retry is safe to make unconditionally. A `409` means something the caller cannot fix by trying again, such as no certificate for that subject.
...
The cost falls on revocation, and it is worth sizing honestly. A revoke waits for whatever issuance holds the lock for that subject, and that issuance may itself be queued on the cluster-wide CRL lock, so the wait is not merely "one signature" — on a busy CA it can consume the whole 60 seconds and give up. That answer is `503` with `Retry-After`, distinct from the `409` a subject with no certificate gets, because retrying is always safe: revoking a serial already on the CRL is a no-op. How the wait is shared out is the backend's business — etcd and PostgreSQL queue waiters in turn, while the Redis lock is polled, so a revocation competing with a stream of renewals from another replica has no guarantee of being served promptly. If you are containing a compromised agent, the [containment order below](#revocation-cluster-wide) puts closing off issuance first for this reason.
