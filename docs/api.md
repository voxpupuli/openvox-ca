...
If the presented certificate has been revoked, both paths reject the request with `403 Forbidden`. That check reads the CRL from storage rather than the in-memory copy the mTLS layer uses, so it holds even on a replica that has not yet picked up a revocation performed elsewhere — renewal cannot be used to escape a lockout during the propagation window. See [Revocation across replicas](configuration.md#revocation-across-replicas).
