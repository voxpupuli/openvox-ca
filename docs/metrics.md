...
> used to reach a log line only. The paths that take the same lock directly —
> the revoke step inside `Clean`, `Renew` and `AutoRenew`, `ImportCA`, and the
> `crl_chain_file` refresh — are not counted on that arm, though the chain
> refresh counts its own failures on `puppetca_crl_chain_refresh_failures_total`.
>
